...
LAB_00434098:
  ...
  vm->flags = vm->flags & ~AnmVmFlags_13;
...
LAB_00433998:
  ...
  case AnmOpcode_FlipX:
    vm->flags = vm->flags & ~(AnmVmFlags_6|AnmVmFlags_7) | (vm->flags >> 6 & 3 ^ 1) << 6;
    vm->scaleX = vm->scaleX * -1.0;
    break;
  case AnmOpcode_FlipY:
    vm->flags = vm->flags & ~(AnmVmFlags_6|AnmVmFlags_7) | (vm->flags >> 6 & 3 ^ 2) << 6;
  ...
  case AnmOpcode_SetBlendAdditive:
    vm->flags = vm->flags | AnmVmFlags_2;
    break;
  ...
  case AnmOpcode_PosTimeLinear:
    vm->flags = vm->flags & ~(AnmVmFlags_10|AnmVmFlags_11);
    goto LAB_00433f53;
  case AnmOpcode_PosTimeDecel:
    vm->flags = vm->flags & ~(AnmVmFlags_10|AnmVmFlags_11) | AnmVmFlags_10;
    goto LAB_00433f53;
  case AnmOpcode_PosTimeAccel:
    vm->flags = vm->flags & ~(AnmVmFlags_10|AnmVmFlags_11) | AnmVmFlags_11;
LAB_00433f53:
    if ((vm->flags >> 5 & 1) == 0) {
  ...
  case AnmOpcode_23:
    vm->flags = vm->flags | (AnmVmFlags_8|AnmVmFlags_9);
    break;
  case AnmOpcode_StopHide:
    goto switchD_004339dd_caseD_18;
  case AnmOpcode_25:
    vm->flags = vm->flags & ~AnmVmFlags_5 | (curInstr->args[0] & 1) << 5;
    break;
  ...
  case AnmOpcode_SetVisibility:
    vm->flags = vm->flags & ~AnmVmFlags_1 | curInstr->args[0] & AnmVmFlags_1;
    break;
  case AnmOpcode_30:
    vm->scaleInterpFinalX = (float)curInstr->args[0];
    ...
    break;
  case AnmOpcode_31:
    vm->flags = vm->flags & ~AnmVmFlags_12 | (curInstr->args[0] & 1) << 0xc;
  }
  vm->currentInstruction = (AnmRawInstr *)((int)curInstr->args + (uint)curInstr->argsCount);
  goto LAB_00433998;
