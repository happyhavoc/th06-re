  ...
  case STATE_KEYCONFIG:
    ...
    for (i = 0; i < 9; i = i + 1) {
      if (menu->controlMapping[i] < 0) {
        vm_memset->flags = vm_memset->flags & 0xfffffffd;
      }
      else {
...
LAB_0043666d:
  ...
  case STATE_SHOT_SELECT:
    ...
    vm_memset = menu->vmList + 0x5c;
    for (i = 0; i < 2; i = i + 1) {
      vm_memset[1].flags = vm_memset[1].flags | AnmVmFlags_3;
      vm_memset = vm_memset + 2;
    }
    vm_memset = menu->vmList + (uint)g_GameManager.character * 2 + 0x5c;
    for (i = 0; i < 2; i = i + 1) {
      vm_memset->flags = vm_memset->flags | AnmVmFlags_3;
      vm_memset->flags = vm_memset->flags | AnmVmFlags_1;
      if (i == menu->cursor) {
        if ((g_Supervisor.cfg.opts & 1) == 0) {
